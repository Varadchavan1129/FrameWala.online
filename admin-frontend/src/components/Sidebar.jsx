// Sidebar.jsx — Fixed to use AdminAuthContext (not the customer AuthContext)
...
const Sidebar = () => {
  ...
  const adminMenu = [
    ...
    { name: 'Inventory Alerts',path: '/inventory',  icon: FiDatabase },
    { name: 'Reviews',         path: '/reviews',    icon: FiStar },
  ];

  const customerUrl = import.meta.env.VITE_CUSTOMER_URL || 'http://localhost:3000';
  ...
  return (
    <aside className="w-64 bg-slate-900 text-slate-400 border-r border-slate-800 flex flex-col h-screen sticky top-0">
      ...
      <div className="h-16 flex items-center px-6 border-b border-slate-800">
        <Link to="/dashboard" className="flex items-center space-x-2 text-white font-extrabold text-xl tracking-tight">
